...
function Tabla(listaproducto) {
  ...
  for (let i = 0; i < listaproducto.length; i++) {
    document.write("<tr>");
    document.write(`<td style="padding: 12px;">${listaproducto[i].nombreProducto}</td>`);
    document.write(`<td style="padding: 12px;">${listaproducto[i].precio}</td>`);
    document.write(`<td style="padding: 12px;">${listaproducto[i].categoria}</td>`);
    document.write("</tr>");
  }
...
Tabla(listaProductos);
document.write("<hr>")
document.write("<h2>Filtrar  protectores solares</h2>");
let filtroProtectorSolar = listaProductos.filter((producto) =>
  producto.categoria.includes("Protector solar")
);
document.write("<hr>")
Tabla(filtroProtectorSolar);
document.write("<hr>")
document.write("<h2>Busqueda de producto Sérum</h2>");
document.write("<hr>")
let buscarProductoSerum = [
  listaProductos.find((producto) => producto.categoria.includes("Sérum")),
];

Tabla(buscarProductoSerum);
document.write("<hr>")
document.write("<h2>Busqueda de producto Bruma</h2>");
document.write("<hr>")
let buscarProductoBruma = [
  listaProductos.find((producto) => producto.categoria.includes("Bruma")),
];
if (buscarProductoBruma[0] === undefined) {
  document.write("<h3>No se encontro el producto de categoria: Bruma</h3>");
} else {
  Tabla(buscarProductoBruma);
}
